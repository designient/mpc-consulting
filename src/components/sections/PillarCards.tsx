...
function PillarCard({ pillar }: { pillar: PillarItem }) {
  ...
  return (
    <article className="flex flex-col h-full min-w-0 bg-white border-2 border-cta rounded-card overflow-hidden hover:shadow-subtle transition-shadow">
      <div className="relative w-full h-[120px] sm:h-[128px] md:h-[136px] shrink-0 overflow-hidden bg-bg-light">
        ...
        : (
          <div
            className="w-full h-full flex items-center justify-center bg-cta/10"
            aria-hidden>
            <Icon className="w-10 h-10 text-cta/40" />
          </div>
        )}
      </div>
      <div className="flex flex-col flex-grow p-5 md:p-6 min-w-0">
        <Icon className="w-7 h-7 text-cta mb-3 shrink-0" />
        <h3 className="type-h3 text-[18px] md:text-[20px] text-cta mb-2 leading-snug">
          {pillar.title}
        </h3>
        <p className="font-body text-[14px] leading-[1.6] text-cta">
          {pillar.body}
        </p>
      </div>
    </article>
